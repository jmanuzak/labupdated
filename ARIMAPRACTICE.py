...
def get_arima_result(data, column_1, column_2):
	...
	size = int(len(df_new_array) * 0.80)
	...
	for t in range(len(test)):
		model = ARIMA(history, order=(10, 10, 0))
		model_fit = model.fit()
...
def adfuller_test(mortality_rate):
    result=adfuller(mortality_rate)
...
data['fartality_rate'] = data['Deaths'] / data['Recovered']
